...
- Status: Fire TV Settings was debugged at ActivityTaskManager level. The `KEYCODE_SETTINGS` path only opens Quick Settings, and `com.amazon.tv.settings.v2/.tv.device.DeviceActivity` opens a Device/About subsection. OpenCore now intentionally allows a short Amazon launcher settings handoff and launches `com.amazon.tv.launcher/.ui.SettingsActivity`, so Home Guard should not immediately rescue back to OpenCore during that explicit native settings flow.
