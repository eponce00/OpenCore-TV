...
void main() {
  ...
  group("pickWallpaper", () {
    test("picks image", () async {
      ...
      final OpenCoreTVChannel = MockOpenCoreTVChannel();
      final settingsService = MockSettingsService();
      when(imagePicker.pickImage(source: ImageSource.gallery))
          .thenAnswer((_) => Future.value(pickedFile));
      when(OpenCoreTVChannel.checkForGetContentAvailability())
          .thenAnswer((_) => Future.value(true));
      final wallpaperService =
          WallpaperService(OpenCoreTVChannel, settingsService);
    ...
    test("throws error when no file explorer installed", () async {
      ...
      final wallpaperService =
          WallpaperService(OpenCoreTVChannel, MockSettingsService());
      await untilCalled(pathProviderPlatform.getApplicationDocumentsPath());
  ...
  test("setGradient", () async {
    ...
    await untilCalled(pathProviderPlatform.getApplicationDocumentsPath());
    await wallpaperService.setGradient(OpenCoreTVGradients.greatWhale);

    verify(
        settingsService.setGradientUuid(OpenCoreTVGradients.greatWhale.uuid));
  ...
  group("getGradient", () {
    test("without uuid from settings", () async {
      ...
      expect(gradient, OpenCoreTVGradients.greatWhale);
    });
    ...
    test("with uuid from settings", () async {
      ...
      when(settingsService.gradientUuid)
          .thenReturn(OpenCoreTVGradients.grassShampoo.uuid);
      await untilCalled(pathProviderPlatform.getApplicationDocumentsPath());

      final gradient = wallpaperService.gradient;

      expect(gradient, OpenCoreTVGradients.grassShampoo);
    });
  });
}
