...
class UpdateService {
  static const String _owner = "eponce00";
  static const String _repo = "opencore-tv";
  final OpenCoreTVChannel _openCoreTVChannel;

  UpdateService({OpenCoreTVChannel? channel})
      : _openCoreTVChannel = channel ?? OpenCoreTVChannel();
  ...
  Future<DownloadedApk> downloadApk(UpdateResult update) async {
    ...
    try {
      ...
      request.headers.set(HttpHeaders.userAgentHeader, "opencore-tv-Updater");
      ...
      final fileName =
          update.apkName ?? "opencore-tv-${update.latestVersion}.apk";
  ...
  Future<bool> installApk(String apkPath) {
    return _openCoreTVChannel.installApk(apkPath);
  }

  Future<void> requestInstallUnknownAppsPermission() {
    return _openCoreTVChannel.requestInstallUnknownAppsPermission();
  }
