...
class AppsService extends ChangeNotifier {
  static const _launcherPackageNames = {
    ...
    'tv.opencore.launcher.debug',
  };

  static const _defaultFavoritePackageNames = [
    'org.jellyfin.androidtv',
    'com.amazon.firetv.youtube',
    'com.google.android.youtube.tv',
  ];

  static const _syntheticInputPackagePrefix = 'opencore.input.';

  final OpenCoreTVChannel _OpenCoreTVChannel;
  final OpenCoreTVDatabase _database;
  ...
  void invalidateAppImage(String packageName) {
  ...
  AppsService(this._OpenCoreTVChannel, this._database) {
    _init();
  }
  ...
  Future<void> _init() async {
    ...
    _OpenCoreTVChannel.addAppsChangedListener((event) async {
    ...
    return _database.transaction(() async {
      if (allApps.isNotEmpty) {
        ...
        for (final app in allApps) {
          await addToCategory(app, allAppsCategory,
              shouldNotifyListeners: false);
        }
      }
      ...
      for (final packageName in [
        ..._defaultFavoritePackageNames,
        ..._launcherPackageNames,
      ]) {
        final app = _applications[packageName];
        if (app != null && !app.hidden) {
          await addToCategory(app, favoritesCategory,
              shouldNotifyListeners: false);
  ...
  Future<void> _refreshState({bool shouldNotifyListeners = true}) async {
    ...
    List<Map<dynamic, dynamic>> appsFromSystem =
        await _OpenCoreTVChannel.getApplications();
    ...
    for (App app in appsRemovedFromSystem) {
      ...
      bool appExists = await _OpenCoreTVChannel.applicationExists(packageName);
      if (!appExists) {
    ...
    await _ensureSyntheticInputsVisible();
    await _ensureEssentialFavorites();
  ...
  Future<void> _ensureEssentialFavorites() async {
    Category? favorites;
    ...
    final hasRealFavorite = favorites.applications
        .any((app) => !_launcherPackageNames.contains(app.packageName));
    if (hasRealFavorite) return;

    for (final packageName in _defaultFavoritePackageNames) {
      final app = _applications[packageName];
      if (app == null || app.hidden) continue;

      final alreadyPinned = favorites.applications
          .any((favorite) => favorite.packageName == packageName);
      if (!alreadyPinned) {
  ...
  Future<void> _ensureSyntheticInputsVisible() async {
    final allAppsCategory = _getAppsCategory();
    if (allAppsCategory == null) return;

    final syntheticInputs = _applications.values.where(
      (app) =>
          app.packageName.startsWith(_syntheticInputPackagePrefix) &&
          !app.hidden,
    );

    for (final app in syntheticInputs) {
      final isAlreadyInAnyCategory = _categoriesById.values.any(
        (category) => category.applications.any(
          (categoryApp) => categoryApp.packageName == app.packageName,
        ),
      );

      if (!isAlreadyInAnyCategory) {
        await addToCategory(app, allAppsCategory, shouldNotifyListeners: false);
      }
    }

    sortCategory(allAppsCategory);
  }
  ...
  Future<Uint8List> getAppBanner(String packageName) async {
    ...
    if (!packageName.startsWith(_syntheticInputPackagePrefix)) {
      try {
    ...
    final bytes = await _OpenCoreTVChannel.getApplicationBanner(packageName);
    if (bytes.isNotEmpty) {
      _bannerCache[packageName] = bytes;
    }
    return bytes;
  ...
  Future<Uint8List> getAppIcon(String packageName) async {
    ...
    final bytes = await _OpenCoreTVChannel.getApplicationIcon(packageName);
    if (bytes.isNotEmpty) {
  ...
  Future<void> launchApp(App app) async {
    ...
    Future<void> future;
    if (app.action == null) {
      future = _OpenCoreTVChannel.launchApp(app.packageName);
    } else {
      future = _OpenCoreTVChannel.launchActivityFromAction(app.action!);
    }
  ...
  Future<void> uninstallApp(App app) =>
      _OpenCoreTVChannel.uninstallApp(app.packageName);
  ...
  Future<bool> openAccessibilitySettings() =>
      _OpenCoreTVChannel.openAccessibilitySettings();

  Future<bool> isDefaultLauncher() => _OpenCoreTVChannel.isDefaultLauncher();
